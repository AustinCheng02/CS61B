...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            return move(move0.col0(), move0.row0(),
                    move1.col0(), move1.row0(), move1._nextJump);
        }
        if (move0.jumpTail() == null) {
            return move(move0.col0(), move0.row0(),
                    move0.col1(), move0.row1(), move1);
        } else {
            Move newJump = move(move0._nextJump, move1);
            return move(move0.col0(), move0.row0(),
                    move0.col1(), move0.row1(), newJump);
        }
     ...
     *  the left. */
    boolean isLeftMove() {
        return !_isJump && (_col0 > _col1) && (_row0 == _row1);
    }
    ...
    private void toString(Formatter out) {
        if (_nextJump == null) {
            out.format("%1$s%2$s-%3$s%4$s", col0(), row0(), col1(), row1());
        } else {
            Move jump = _nextJump;
            out.format("%1$s%2$s-%3$s%4$s", col0(), row0(), col1(), row1());
            while (jump != null) {
                out.format("-%1$s%2$s", jump.col1(), jump.row1());
                jump = jump.jumpTail();
            }
        }
    }
