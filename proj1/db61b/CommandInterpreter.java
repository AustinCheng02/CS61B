...
class CommandInterpreter {
    ...
    void insertStatement() {
        ...
        _input.next(")");
        table.add(inserts.toArray(new String[inserts.size()]));
        while (_input.nextIf(",")) {
        ...
        if (inserts.size() != table.columns()) {
            skipCommand();
            throw new DBException("Inputs not equal to column length.");
        }
        _input.next(";");
    }
    ...
    void loadStatement() {
        ...
        Table loadT = Table.readTable(name);
        _database.put(name, loadT);
    ...
    void printStatement() {
        _input.next("print");
        String title = _input.peek();
        Table print = tableName();
        _input.next(";");
        System.out.printf("Contents of " + title + ":");
        System.out.println();
        print.print();
    }
    ...
    Table selectClause() {
        ...
        if (_input.nextIf("where")) {
            Table[] tableArgs = {table1};
            table = table1.select(columns, conditionClause(tableArgs));
            return table;
        } else if (_input.nextIf(",")) {
            table2 = tableName();
            if (_input.nextIs(";")) {
                List<Condition> cond = new ArrayList<>();
                table = table1.select(table2, columns, cond);
            } else {
                _input.next("where");
                Table[] tableArgs = {table1, table2};
                table = table1.select(table2, columns,
                        conditionClause(tableArgs));

            }
        } else {
            List<Condition> cond = new ArrayList<>();
            table = table1.select(columns, cond);
        }
        return table;
    }
    ...
    Condition condition(Table... tables) {
        ...
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String val2 = _input.next(Tokenizer.LITERAL);
            val2 = val2.substring(1, val2.length() - 1).trim();
            return new Condition(col1, rel, val2);
        } else {
            Column col2 = new Column(name(), tables);
