...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        double[][] accum = new double[height][width];
        double minim;
        accum[0] = m[0];
        for (int i = 1; i < height; i++) {
            for (int j = 0; j < width; j++) {
                minim = NumberUtils.min(get(accum, i-1, j-1), get(accum, i-1, j), get(accum, i-1, j+1));
                accum[i][j] = minim + m[i][j];
            }
        }
        return accum;
    }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        int newWidth = m.length;
        double[][] horizon = new double[newHeight][newWidth];
        for (int i = 0; i < newHeight; i++) {
            for (int j = 0; j < newWidth; j++) {
                horizon[i][j] = m[Math.abs(j-newHeight-1)][i];
            }
        }
        return accumulateVertical(horizon);
    }
